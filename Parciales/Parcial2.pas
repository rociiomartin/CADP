...
C) Implementar un módulo que elimine, de la estructura generada en "A)", la venta con código recibido por parámetro. Tener en cuenta que la venta puede no existir. El módulo debe retornar si pudo o no eliminarse la
venta.
...
begin
    ...
    begin
        ...
        begin
            ...
            if (l^.datos.f.dia = 1)then v[l^.datos.f.mes]:=v[l^.datos.f.mes]+1;
            if ( (l^.datos.f.mes = 2 ) and (l^.datos.f.mes = 9) and (Cumple (l^.datos.dni)) ) then cant:=cant+1;
            cantT:=cantT+1;
            l:=l^.sig;
        end;  
